...
const ChatRoom = ({ roomId }) => {
    ...
    useEffect(() => {
        ...
        socket.on("receive-message", (data) => {
            ...
            if (notificationTone.current && data.senderId !== userId) {
    ...
    const searchYouTube = async (query) => {
        ...
        try {
            ...
            const response = await fetch(
                `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&q=${encodedQuery}&key=${API_KEY}`
            );
        ...
        <>
            ...
            {modal && (
                <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-80 z-50">
                    <div className="bg-gray-900 p-6 rounded-lg w-11/12 max-w-2xl max-h-[80vh] overflow-y-auto">
                        <button
                                ...
                                {searchResults.length > 0 ? (
                                    searchResults.map((video) => (
                                        <div
                                            ...
                                            className="flex items-center justify-between mb-2 p-2 hover:bg-gray-800 rounded"
                                        >
                                            <div className="text-white flex-1 truncate mr-4">
                                                {video.snippet.title}
                                            </div>
                                            <button
